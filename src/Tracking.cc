...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
                   ...
                   const bool bReuse)
    : mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(bReuse),
...
void Tracking::MonocularInitialization() {
  ...
  if (!mpInitializer) {
    ...
    if (mCurrentFrame.mvKeys.size() > 100) {
      ...
      cout << __FUNCTION__
           << "The Key Frame-s points are less: " << mCurrentFrame.mvKeys.size()
           << endl;
  } else {
    // Try to initialize
    if ((int)mCurrentFrame.mvKeys.size() <= 100) {

      // cout << __FUNCTION__ << "old.The Key Frame-s points are less: " <<
      // mCurrentFrame.mvKeys.size() << endl;
      delete mpInitializer;
    ...
    if (nmatches < 100) {
      cout << __FUNCTION__
           << "ORB extraction : No enough correspondesnces(<100) " << nmatches
           << endl;
      delete mpInitializer;
      mpInitializer = static_cast<Initializer *>(NULL);
      return;
...
void Tracking::CreateInitialMapMonocular() {
  ...
  if (medianDepth < 0 || pKFcur->TrackedMapPoints(1) < 100) {
    cout << "Wrong initialization, reseting... map points(100):"
         << pKFcur->TrackedMapPoints(1) << " medianDepth = " << medianDepth
         << endl;
    Reset();
    return;
  }
  ...
  while (nCandidates > 0 && !bMatch) {
    ...
    for (int i = 0; i < nKFs; i++) {
      ...
      if (!Tcw.empty()) {
        ...
        // again
        if (nGood < 50) {
          // cout << "Relocalization:  inliers < 50 : nGood = " << nGood <<
          // endl;
